...
function FAQItemWhite({ item, index, isOpen, toggle }) {
  ...
  return (
    <div className="bg-gray-50 border border-gray-200 rounded-xl overflow-hidden hover:bg-gray-100 hover:border-gray-300 transition-all duration-300 group">
      <button
        onClick={() => toggle(index)}
        className="w-full cursor-pointer text-left flex justify-between items-center p-5 lg:p-6 font-semibold text-gray-900 hover:text-gray-700 transition-colors"
      >
        <span className="text-base md:text-lg pr-4">{item.question}</span>
        <FontAwesomeIcon 
          icon={isOpen ? faAngleUp : faAngleDown} 
          className={`ml-2 text-[#193555] transition-all duration-300 flex-shrink-0 ${isOpen ? 'rotate-180' : 'rotate-0'}`}
        />
      </button>
      <div
      ...
      >
        <div ref={contentRef}>
          <p className="text-gray-600 text-sm md:text-base px-5 lg:px-6 pb-5 lg:pb-6 leading-relaxed border-t border-gray-200 pt-4">
            {item.answer}
          </p>
        </div>
      </div>
...
export default function FAQWhite() {
  ...
  return (
    <div className="faq-section-white px-[2%] sm:px-[8%] lg:px-[12%] py-[50px] lg:py-[90px] bg-white relative overflow-hidden">
      {/* Subtle background pattern (optional) */}
      <div className="absolute inset-0 opacity-5">
        <div className="absolute top-10 left-10 w-72 h-72 bg-[#193555] rounded-full blur-3xl"></div>
        <div className="absolute bottom-10 right-10 w-96 h-96 bg-gray-300 rounded-full blur-3xl"></div>
      </div>

      <div className="relative z-10">
        <div className="text-center mb-12">
          <h2 className="text-gray-900 text-3xl md:text-4xl xl:text-5xl font-bold mb-4 unbounded-font leading-tight">
        ...
        <div className="text-center mt-12 pt-8 border-t border-gray-200">
          <p className="text-gray-600 mb-4">Still have questions? Connect With Our Support Center</p>
          <button className="btn bg-[#f3f3f3] group hover:bg-[#193555] font-bold px-8 py-3 rounded-full cursor-pointer transition-all duration-300 shadow-lg hover:shadow-xl">
